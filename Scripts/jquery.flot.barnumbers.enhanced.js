/**
 Look for RQCHANGES to see what was edited by RadiantQ
    ...
    var options = {
        bars: {
            numbers: {
                show: false,
                threshold: false,
                thresholdColor: false,//RQCHANGES
                yOffset: 0,
                xOffset: 0
            }
        }
    ...
    function draw(plot, ctx)
    {
        ...
        $.each(plot.getData(), function(index, series)
        {
            ...
            for (i = 0; i < points.length; i += series.datapoints.pointsize)
            {
                ...
                // RQCHANGES Starts
                if (barnumbers.thresholdColor) {
                    if ((points[barNumber] > ((plot.getAxes().yaxis.max * barnumbers.thresholdColor) + (point.y / 6)))) {
                        ctx.fillStyle = 'rgb(240 , 240, 240)';
                        point.y = point.y;
                    }
                    else {
                        ctx.fillStyle = barnumbers.fontColor;
                        var min = points[i + 1] + (point.y / 10); //Calculate the min value that is overlap with the bar
                        if (min > point.y) {
                            point.y = min;
                        }
                    }
