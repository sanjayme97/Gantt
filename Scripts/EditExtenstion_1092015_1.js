...
function templateCallBack(tableRow, data) {
	...
	if ($startTimeCol.length == 1 && $startTimeCol[0]["alreadyBound"] == undefined) {
	    ...
	    $startTimeCol.append(data.Activity["StartTime"].toString("yyyy-MM-dd"));
    }  
	if ($endTimeCol.length == 1 && $endTimeCol[0]["alreadyBound"] == undefined) {

	    $endTimeCol.append(data.Activity["EndTime"].toString("yyyy-MM-dd"));
    }
        ...
        switch (args.PropertyName) {
            ...
            case "EndTime":
                if ($endTimeCol[0]["alreadyBound"] == undefined)
                    $endTimeCol.html(sender["EndTime"].toString("yyyy-MM-dd"));
                break;
            case "StartTime":
                if ($startTimeCol[0]["alreadyBound"] == undefined)
                    $startTimeCol.html(sender["StartTime"].toString("yyyy-MM-dd"));
                break;
